-- Update blog posts with diverse international authors, real avatar images, and cover images

UPDATE blog_posts SET
  author_name = 'Sarah Mitchell',
  author_avatar = 'https://randomuser.me/api/portraits/women/44.jpg',
  cover_image = 'https://images.unsplash.com/photo-1456513080510-7bf3a84b82f8?w=800&h=400&fit=crop'
WHERE slug = 'spaced-repetition-science';

UPDATE blog_posts SET
  author_name = 'James Rodriguez',
  author_avatar = 'https://randomuser.me/api/portraits/men/32.jpg',
  cover_image = 'https://images.unsplash.com/photo-1677442136019-21780ecad995?w=800&h=400&fit=crop'
WHERE slug = 'ai-powered-studying';

UPDATE blog_posts SET
  author_name = 'Aisha Patel',
  author_avatar = 'https://randomuser.me/api/portraits/women/68.jpg',
  cover_image = 'https://images.unsplash.com/photo-1434030216411-0b793f4b4173?w=800&h=400&fit=crop'
WHERE slug = 'effective-flashcard-techniques';

UPDATE blog_posts SET
  author_name = 'Marcus Chen',
  author_avatar = 'https://randomuser.me/api/portraits/men/75.jpg',
  cover_image = 'https://images.unsplash.com/photo-1522202176988-66273c2fd55f?w=800&h=400&fit=crop'
WHERE slug = 'live-quiz-classroom';

UPDATE blog_posts SET
  author_name = 'Elena Kowalski',
  author_avatar = 'https://randomuser.me/api/portraits/women/52.jpg',
  cover_image = 'https://images.unsplash.com/photo-1501504905252-473c47e087f8?w=800&h=400&fit=crop'
WHERE slug = 'exam-preparation-guide';

UPDATE blog_posts SET
  author_name = 'David Nakamura',
  author_avatar = 'https://randomuser.me/api/portraits/men/22.jpg',
  cover_image = 'https://images.unsplash.com/photo-1455390582262-044cdead277a?w=800&h=400&fit=crop'
WHERE slug = 'handwriting-ocr-digital-notes';

UPDATE blog_posts SET
  author_name = 'Fatima Al-Hassan',
  author_avatar = 'https://randomuser.me/api/portraits/women/85.jpg',
  cover_image = 'https://images.unsplash.com/photo-1460925895917-afdab827c52f?w=800&h=400&fit=crop'
WHERE slug = 'study-analytics-track-progress';

UPDATE blog_posts SET
  author_name = 'Lucas Bergström',
  author_avatar = 'https://randomuser.me/api/portraits/men/46.jpg',
  cover_image = 'https://images.unsplash.com/photo-1509062522246-3755977927d7?w=800&h=400&fit=crop'
WHERE slug = 'teach-back-method';

UPDATE blog_posts SET
  author_name = 'Priya Sharma',
  author_avatar = 'https://randomuser.me/api/portraits/women/33.jpg',
  cover_image = 'https://images.unsplash.com/photo-1457369804613-52c61a468e7d?w=800&h=400&fit=crop'
WHERE slug = 'deep-research-essays';

UPDATE blog_posts SET
  author_name = 'Thomas Weber',
  author_avatar = 'https://randomuser.me/api/portraits/men/64.jpg',
  cover_image = 'https://images.unsplash.com/photo-1513258496099-48168024aec0?w=800&h=400&fit=crop'
WHERE slug = 'getting-started-studyrpg';
